...
import { setAuth, signup } from "@/lib/api"
      ...
      <main className="flex flex-1 items-center justify-center px-6 py-12">
        <div className="w-full max-w-md">
          <div className="mb-8 flex flex-col gap-1">
            <span className="text-xs font-semibold uppercase tracking-[0.2em] text-muted-foreground">Créer mon profil</span>
            <h1 className="text-balance font-serif text-2xl font-semibold text-foreground md:text-3xl">Ouvrez votre registre de confiance</h1>
          </div>
          <form className="border-2 border-foreground/80 bg-card p-6 md:p-8" onSubmit={handleSubmit}>
            <FieldGroup>
              <Field><FieldLabel htmlFor="name">Nom complet</FieldLabel><Input id="name" name="name" placeholder="Ex : Awa Ndiaye" required /></Field>
              <Field><FieldLabel htmlFor="whatsapp">Numéro WhatsApp</FieldLabel><Input id="whatsapp" name="whatsapp" type="tel" placeholder="+221 77 000 00 00" required /></Field>
          ...
          <p className="mt-4 text-center text-sm text-muted-foreground">Déjà un profil ? <Link href="/login" className="font-medium text-primary underline-offset-4 hover:underline">Se connecter</Link></p>
        </div>
      </main>
    </div>
  )
