...
import { getProfile, getStoredUser, updateMyProfile, type ApiProfile } from "@/lib/api"
...
export default function DashboardPage() {
  ...
  async function handleSave() {
    ...
    setSuccess("")
    try {
      const updated = await updateMyProfile({ name: name.trim(), bio: bio.trim(), whatsapp: whatsapp.trim() })
      setProfile(updated)
  ...
  if (loading) {
    return <div className="flex min-h-svh items-center justify-center text-sm text-muted-foreground">Chargement du profil…</div>
  }

  if (!profile) {
    return <div className="flex min-h-svh items-center justify-center px-6 text-sm text-destructive">{error || "Profil introuvable."}</div>
  }
        ...
        <div className="grid gap-8 lg:grid-cols-[1fr_1.1fr]">
          <div className="flex flex-col gap-8">
            ...
            <section className="border border-border bg-secondary/40 p-6">
              ...
              <div className="flex items-center gap-2">
                <code className="flex-1 truncate border border-border bg-card px-3 py-2 text-sm text-foreground/80">confiance.app{publicPath}</code>
          ...
          <div className="flex flex-col gap-8">
            ...
            <section className="border-2 border-foreground/80 bg-card p-6 md:p-8">
              ...
              <div>{profile.reviews.map((review, index) => <LedgerReview key={review.id} review={review} rotate={index % 2 === 0 ? -5 : 4} />)}</div>
            </section>
          </div>
        </div>
      </main>
